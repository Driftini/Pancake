$file.delete("/a/.pancake/temp/")
$explorer.refresh()

document.getElementById("s42_start").children[0].src = "data:image/png;base64,"

let pancakeDashboard = $file.getUrl("/a/.pancake/dashboard/dashboard.html",function(e){
    le._apps.pancake = {
        exec: function() {
            $window(
                {
                    url: e,
                    title: "Pancake Dashboard",
                    dockable: false,
                    minimizable: false,
                    width: 666,
                    height: 423,
                    baseClass: "dashboardWindow",
                    icon: "data:image/png;base64,"
                }
            )
        },
        hascli: true,
        terminal: true
    }
})

var pancakeConfig = $file.open("/a/.pancake/config.json","String",function(e){pancakeConfig = e})

let pancakeTextbox = document.createElement("p")
pancakeTextbox.classList = ["pancaketext"]
pancakeTextbox.innerHTML = "<b>Driftini (not) Confidential</b><br>Unauthorized use or disclosure in any manner may result in disciplinary action up to and including denomination as monkey, prohibition of access to ifunny.co and a permanent ban from the Windows93 unofficial Minecraft server.<br><p>Pancake Beta 1</p>"
document.getElementById("s42_desktop").appendChild(pancakeTextbox)